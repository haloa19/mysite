				...
				<div class="pager">
					<ul>
						<c:choose>
							...
							<c:otherwise><li><a href="${pageContext.servletContext.contextPath }/board?a=list&page=${nowPage-1 }&bpage=${beginPage}&kwd=${keyword}">◀</a></li></c:otherwise>
						</c:choose>		
							<c:forEach var="i" begin="${beginPage }" end="${endPage}">
								<c:choose>
									<c:when test="${nowPage == i }">
										<li class="selected"><a href="${pageContext.servletContext.contextPath }/board?a=list&page=${i }&kwd=${keyword}">${i }</a></li>
									</c:when>
									<c:otherwise>
										<li><a href="${pageContext.servletContext.contextPath }/board?a=list&page=${i }&kwd=${keyword}">${i }</a></li>
									</c:otherwise>			
								</c:choose>	
