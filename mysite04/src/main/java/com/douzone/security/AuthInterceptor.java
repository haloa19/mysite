...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(
		...
		// 4. Method에 @Auth가 없으면...Type(class)에 붙어 있는지 확인한다??? (과제)
		if(auth == null) {
		...
		// 7. @Auth의 role이 "USER"인 경우에는 authUser의 role이 "USER" or "ADMIN" 상관이 없음
		if("USER".equals(role)) {
			return true;
		}
		
		// 8. @Auth의 role이 "ADMIN"인 경우에는 반드시 authUser의 role이 "ADMIN"여야 하는데 유저인 경우 막는 처리
